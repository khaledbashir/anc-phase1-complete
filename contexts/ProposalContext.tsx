...
import type {
    ...
    Exception as VerificationException,
...
const defaultProposalContext = {
    ...
    answerRfpQuestion: (questionId: string, answer: string) =>
        Promise.resolve(),
    filterStats: null as {
...
export const ProposalContextProvider = ({
    ...
    const [proposalPdfLoading, setProposalPdfLoading] =
        useState<boolean>(false);
    const [pdfGenerationProgress, setPdfGenerationProgress] = useState<{
        value: number;
        label: string;
    } | null>(null);
    const [pdfBatchProgress, setPdfBatchProgress] = useState<{
        current: number;
        total: number;
        label: string;
    } | null>(null);
    const [excelImportLoading, setExcelImportLoading] =
        useState<boolean>(false);
    const [excelPreviewLoading, setExcelPreviewLoading] =
        useState<boolean>(false);
    ...
    const [verificationManifest, setVerificationManifest] =
        useState<VerificationManifest | null>(null);
    const [verificationExceptions, setVerificationExceptions] = useState<
    ...
    const [lowMarginAlerts, setLowMarginAlerts] = useState<
        Array<{ name: string; marginPct: number }>
    >([]);
    ...
    const [verifiedFields, setVerifiedFields] = useState<
        Record<string, { verifiedBy: string; verifiedAt: string }>
    >({});
    const [aiFieldTimestamps, setAiFieldTimestamps] = useState<
    ...
    const setFieldVerified = useCallback(
        async (fieldPath: string, userName: string) => {
            // Optimistic Update
    ...
    // RFP Vault Delete
    const deleteRfpDocument = useCallback(
        async (id: string) => {
            try {
    ...
    const [calculationMode, setCalculationModeState] = useState<
        "MIRROR" | "INTELLIGENCE"
    ...
    const setCalculationMode = useCallback(
        (mode: "MIRROR" | "INTELLIGENCE") => {
            ...
            setValue("details.mirrorMode", mode === "MIRROR", {
                shouldValidate: true,
                shouldDirty: true,
            });
        },
    ...
    const isFieldGhostActive = useCallback(
    ...
    const setHeaderType = useCallback(
        (next: "LOI" | "PROPOSAL" | "BUDGET") => {
            ...
            if (next === "LOI") {
                ...
                setValue("details.showPaymentTerms", true, {
                    shouldDirty: true,
                });
                ...
                if (!mirrorMode) {
                    setValue("details.showExhibitA", true, {
                        shouldDirty: true,
                    });
                }
            ...
            setValue("details.documentType", "First Round", {
                shouldValidate: true,
                shouldDirty: true,
            });
            setValue(
                "details.pricingType",
                next === "PROPOSAL" ? "Hard Quoted" : "Budget",
    ...
    useEffect(() => {
        ...
        if (typeof window !== "undefined") {
            ...
            const savedProposalsJSON =
                window.localStorage.getItem("savedProposals");
            try {
                savedProposalsDefault =
                    savedProposalsJSON && savedProposalsJSON.trim() !== ""
                        ? JSON.parse(savedProposalsJSON)
                        : [];
            } catch (e) {
                console.error(
                    "Error parsing savedProposals from localStorage:",
                    e,
                );
                savedProposalsDefault = [];
            }
        ...
        const subscription = watch(() => {
            ...
            try {
                ...
                if (
                    ...
                    JSON.stringify(risksRef.current)
                ) {
    ...
    useEffect(() => {
        ...
        aiCitations,
        verifiedFields,
    ]); // Re-run when screens, AI state, or global settings change
    ...
    useEffect(() => {
        ...
        try {
            ...
            const { clientSummary, internalAudit } = calculateProposalAudit(
                normalized,
                {
            ...
            if (
                JSON.stringify(currentAudit?.totals) !==
                JSON.stringify(internalAudit.totals)
            ) {
            ...
            if (index >= 0 && index < screens.length) {
                ...
                try {
                    const { clientSummary, internalAudit } =
                        calculateProposalAudit(updatedScreens, {
                            taxRate: getValues("details.taxRateOverride"),
                            bondPct: getValues("details.bondRateOverride"),
                            structuralTonnage: getValues(
                                "details.metadata.structuralTonnage",
    ...
    const newProposal = useCallback(
        (opts?: { silent?: boolean }) => {
            ...
            setValue("marginAnalysis" as any, undefined, {
                shouldDirty: false,
            });
    ...
    const generatePdf = useCallback(
        async (data: ProposalType): Promise<Blob> => {
            ...
            try {
                ...
                console.log(
                    "Audit computed for PDF:",
                    audit.internalAudit.totals,
                );
                ...
                const grandTotal =
                    audit?.internalAudit?.totals?.finalClientTotal ?? 0;
                if (Number(grandTotal) === 0 && screens.length > 0) {
                    console.warn(
                        "[PDF] Data Mapping Failed: Pricing not found. Project Total is $0.",
                    );
                    showError(
                        "Data Mapping Failed",
                        "Pricing not found. Project Total is $0. Check Excel mapping or screen pricing.",
                    );
                }
                ...
                if (
                    errorMsg.includes("libnspr4") ||
                    errorMsg.includes("chromium") ||
                    errorMsg.includes("browser process")
                ) {
                    showError(
    ...
    const TEMPLATES = [{ id: 5, label: "ANC Standard (Hybrid)" }] as const;
    const MODES = [
        ...
        for (const { mode, label: modeLabel } of MODES) {
            for (const { id: templateId, label: templateLabel } of TEMPLATES) {
                ...
                try {
                    ...
                    console.error(
                        `PDF variant ${modeLabel} ${templateLabel} failed:`,
                        e,
                    );
                    setPdfBatchProgress(null);
                    showError(
                        "Download All PDFs",
                        `Failed to generate ${modeLabel} ${templateLabel}. Try again or use single PDF download.`,
                    );
                    return;
                }
            }
    ...
    const downloadBundlePdfs = useCallback(async () => {
        ...
        try {
            ...
            const isMirror =
                !!getValues()?.details?.mirrorMode ||
                getValues()?.details?.calculationMode === "MIRROR";

            const excelRes = await fetch("/api/proposals/export/audit", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    proposalId: data.details?.proposalId || "new",
                    projectAddress,
                    venue: data.details?.venue ?? "",
                    internalAudit: data.details?.internalAudit ?? null,
                    screens,
    ...
    const saveProposalData = () => {
        if (proposalPdf) {
            ...
            if (getValues) {
                ...
                const savedProposalsJSON =
                    localStorage.getItem("savedProposals");
                let savedProposals = [];
                try {
                    savedProposals =
                        savedProposalsJSON && savedProposalsJSON.trim() !== ""
                    ...
                    console.error(
                        "Error parsing savedProposals from localStorage during save:",
                        e,
                    );
                    savedProposals = [];
                }
                ...
                const id =
                    formValues?.details?.proposalId || `prop-${Date.now()}`;
                ...
                try {
                    const screens = (formValues?.details?.screens || []).map(
                        (s: any) => ({
                            name: s.name,
                            productType: s.productType ?? "",
                            heightFt: s.heightFt ?? s.height ?? 0,
                    ...
                    console.warn(
                        "Failed to calculate audit for saved proposal:",
                        e,
                    );
                }
                ...
                try {
                    const workspaceId = (formValues as any)?.details
                        ?.workspaceId;
                    if (workspaceId) {
                        // Prepare minimal payload expected by server
                        const screens =
                            (formValues as any)?.details?.screens || [];
                        fetch("/api/proposals/create", {
                            ...
                            body: JSON.stringify({
                                workspaceId,
                                clientName:
                                    (formValues as any)?.receiver?.name ||
                                    (formValues as any)?.details?.clientName ||
                                    "",
                                screens,
                            }),
                        })
                            .then((res) => {
                                if (!res.ok) {
                                    console.warn(
                                        "Server save failed for proposal",
                                        res.statusText,
                                    );
                                }
                            })
